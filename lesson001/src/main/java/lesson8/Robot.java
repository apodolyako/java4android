...
public class Robot implements Sportmen{
    ...
    public Robot(int max_length_run, int max_height_jump) {
    ...
    public void run(Equipment eq){
        ...
        if(eq instanceof Track){
            ...
            if(max_length_run >=run){
                System.out.println("Робот успешно пробежал "+ run + "м");
            }else{
                System.out.println( "Робот не смог пробежать " + run + "м, тк по силам только: " + max_length_run+"м" );
                flagIsInGame = false;
                printIamOut();
            }
        ...
        if (eq instanceof Track) {
           // System.out.println("Робот не прыгнул тк нужно бежать ");
            ...
            if(max_height_jump >=jump){
                System.out.println("Робот успешно прыгнул через стену высотой "+ jump + "м");
            }else{
                System.out.println( "Робот не смог прыгнуть на  " + jump + "м, тк по силам только: " + max_height_jump+"м" );
                flagIsInGame = false;
                printIamOut();
            }
        }
    ...
    private void printIamOut(){
        System.out.println("======Робот выходит из соревнования... ((((");
    }
}
